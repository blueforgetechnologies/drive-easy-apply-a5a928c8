...
export default function AuditDetail() {
  ...
  const { data: loads } = useQuery({
    queryKey: ["ready-for-audit-loads-nav"],
    queryFn: async () => {
  ...
  const currentIndex = loads?.findIndex((l) => l.id === id) ?? -1;
  const totalLoads = loads?.length ?? 0;

  const navigateToLoad = (index: number) => {
    if (loads && index >= 0 && index < loads.length) {
      navigate(`/dashboard/accounting/audit/${loads[index].id}`);
    }
  };
