...
export default function InvoicesTab() {
  ...
  const getBillingMethodBadge = (method: string | null) => {
    switch (method) {
      case 'otr':
        return <span className="text-xs font-bold text-green-800 dark:text-green-400">OTR Solutions</span>;
      case 'direct_email':
  ...
  const getCreditApprovalBadge = (inv: InvoiceWithDeliveryInfo) => {
    ...
    switch (inv.credit_approval_status) {
      case 'approved':
        return wrapWithSource(<span className="text-xs font-bold text-green-800 dark:text-green-400">Approved</span>);
      case 'denied':
