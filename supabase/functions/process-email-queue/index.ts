...
function parseSylectusEmail(subject: string, bodyText: string): Record<string, any> {
  ...
  // Helper to parse MM/DD/YYYY HH:MM AM/PM TZ format (Sylectus style)
  ...
  if (data.posted_at && data.expires_at) {
    ...
    if (expiresTime <= postedTime) {
      ...
      console.log(`📅 Corrected expires_at: was before posted_at, now posted_at + 40min -> ${data.expires_at}`);
    }
  }
  ...
  if (data.expires_at) {
    ...
    if (expiresTime < now) {
      ...
      data.expires_at = correctedExpires.toISOString();
      console.log(`📅 Corrected expires_at: was already expired on arrival, now now() + 40min -> ${data.expires_at}`);
    }
  }
...
function parseFullCircleTMSEmail(subject: string, bodyText: string, bodyHtml?: string): Record<string, any> {
  ...
  if (data.posted_at && data.expires_at) {
    ...
    if (expiresTime <= postedTime) {
      const correctedExpires = new Date(postedTime + 40 * 60 * 1000);
      data.expires_at = correctedExpires.toISOString();
      console.log(`📅 FCTMS: Corrected expires_at: was before posted_at, now posted_at + 40min -> ${data.expires_at}`);
    }
  }
  ...
  if (data.expires_at) {
    ...
    if (expiresTime < now) {
      const correctedExpires = new Date(now + 40 * 60 * 1000);
      data.expires_at = correctedExpires.toISOString();
      console.log(`📅 FCTMS: Corrected expires_at: was already expired on arrival, now now() + 40min -> ${data.expires_at}`);
    }
  }
...
serve(async (req) => {
  ...
  try {
    ...
    const matchLoadToHunts = async (
      ...
      if (!tenantId) {
        console.log(`[process-email-queue] BLOCKED - No tenant_id provided, cannot match without tenant context`);
        return;
      }
      ...
      for (const hunt of enabledHunts) {
        ...
        if (hunt.tenant_id !== tenantId) {
          console.error(`[process-email-queue] BLOCKED cross-tenant match: load_email tenant=${tenantId} hunt tenant=${hunt.tenant_id}`);
          continue;
        }
