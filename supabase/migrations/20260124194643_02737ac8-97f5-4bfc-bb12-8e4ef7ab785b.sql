-- Create efficient RPC for dedup cost metrics (platform admin only, read-only)
...
BEGIN
  ...
  -- Count unique content with payload_url present
...
COMMENT ON FUNCTION public.get_dedup_cost_metrics IS 'Returns dedup cost metrics for a given time window. Read-only, platform-admin checked in calling edge function.';