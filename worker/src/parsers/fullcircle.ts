...
export function parseFullCircleTMSEmail(
  ...
  if (!data.posted_at) {
    ...
    console.log(`[fullcircle] Rule 1: posted_at was null, set to now -> ${data.posted_at}`);
  }
  ...
  if (!data.expires_at) {
    ...
    console.log(`[fullcircle] Rule 2: expires_at was null, set to posted_at + 30min -> ${data.expires_at}`);
  } else {
    const expiresTime = new Date(data.expires_at).getTime();
    if (expiresTime <= postedTime) {
      data.expires_at = new Date(postedTime + 30 * 60 * 1000).toISOString();
      console.log(`[fullcircle] Rule 2: expires_at was <= posted_at, set to posted_at + 30min -> ${data.expires_at}`);
    }
  }
  ...
  if (finalExpiresTime < now) {
    ...
    console.log(`[fullcircle] Rule 3: expires_at was already expired, extended to now + 30min -> ${data.expires_at}`);
  }
