...
export function parseSylectusEmail(subject: string, bodyText: string): ParsedEmailData {
  ...
  if (data.posted_at && data.expires_at) {
    ...
    if (expiresTime <= postedTime) {
      ...
      console.log(`[sylectus] Corrected expires_at: was before posted_at, now posted_at + 30min`);
    }
  }
  ...
  if (data.expires_at) {
    ...
    if (expiresTime < now) {
      ...
      data.expires_at = correctedExpires.toISOString();
      console.log(`[sylectus] Corrected expires_at: was already expired on arrival, now now() + 30min -> ${data.expires_at}`);
    }
  }
